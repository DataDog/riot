...
class Interpreter:
    ...
    @classmethod
    def parse(cls, s: str) -> "Interpreter":
        """Parse a string into an interpreter.

        Supports file paths, PATH executables, versions (to be looked up from
        the PATH). Raises FileNotFoundError if no resolution is found.
        """
        if os.path.exists(s):
            return cls(s)
...
class Venv:
    ...
    Args:
        ...
        pys  (List[str]): Python version(s) to use. Overrides parent value.
        ...
        for venv in self.venvs:
            ...
            for env in expand_specs(resolved.env):
                for py in resolved.pys:
                    py_version: Version = Version(str(py))
                    interpreter = None
                    try:
                        interpreter = Interpreter.find_match(py_version)
                    except FileNotFoundError:
                        logger.debug("Failed to find interpreter for %r", py_version)
                    else:
                        logger.debug(
